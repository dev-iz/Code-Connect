...
io.on("connection", (socket) => {
    ...
    socket.on("join-video-chat", () => {
        ...
        socket.emit("all users", userSocketMap);
        socket.broadcast.to(roomId).emit("user-connected-peer", socket.id);
    });
    socket.on("leave-video-chat-peer", (socketId) => {
        const roomId = getRoomId(socketId);
        if (!roomId)
            return;
        socket.broadcast.to(roomId).emit("user-disconnected-peer", socketId);
    });
});
